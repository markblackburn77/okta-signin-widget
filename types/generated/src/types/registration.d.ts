import { SimpleCallback } from './api';
export declare namespace Registration {
    type FieldType = 'string' | 'number' | 'integer' | 'boolean' | 'uri' | 'email' | 'country_code' | 'language_code' | 'locale' | 'timezone' | 'arrayofstring' | 'arrayofnumber' | 'arrayofinteger';
    ...
    interface FieldStringWithFormatAndEnum extends FieldBasic, EnumConstraint<string>, WithDefault<string> {
        type: 'country_code' | 'language_code' | 'locale' | 'timezone';
    }
    ...
    type Field = FieldString | FieldStringWithFormat | FieldStringWithFormatAndEnum | FieldPassword | FieldNumber | FieldBoolean | FieldArray;
    interface Schema {
        lastUpdate: number;
        policyId: string;
        profileSchema: {
    ...
    export interface Callbacks {
        ...
        parseSchema?: (schema: Schema, onSuccess: (schema: Schema) => void, onFailure: (error: Error) => void) => void;
        preSubmit?: (postData: Data, onSuccess: (schema: Data) => void, onFailure: (error: Error) => void) => void;
        postSubmit?: (response: string, onSuccess: (response: string) => void, onFailure: (error: Error) => void) => void;
    }
    export {};
}
